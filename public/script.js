class WebRTCApp {
    ...
    async initializeMedia() {
        try {
            ...
            // Добавляем видео только если есть камера
            if (hasCamera) {
                constraints.video = {
                    width: { ideal: 1280 },
                    height: { ideal: 720 }
                ...
                console.warn('Camera not available, using audio only');
                this.updateStatus('Camera not found - audio only mode');
            }

            this.localStream = await navigator.mediaDevices.getUserMedia(constraints);
            this.localVideo.srcObject = this.localStream;

            // Скрываем видео элемент если нет камеры
            if (!hasCamera) {
                this.localVideo.style.display = 'none';
                document.querySelector('.video-wrapper h3').textContent = 'Audio Only';
            }

            this.updateStatus(hasCamera ? 'Camera and microphone enabled' : 'Microphone enabled (audio only)');
            return true;
        } catch (error) {
            console.error('Error accessing media devices:', error);

            // Пробуем получить только аудио если полный доступ не удался
            try {
                this.localStream = await navigator.mediaDevices.getUserMedia({ audio: true });
                this.localVideo.style.display = 'none';
                document.querySelector('.video-wrapper h3').textContent = 'Audio Only';
                this.updateStatus('Microphone enabled (audio only)');
                return true;
            } catch (audioError) {
    ...
    async shareScreen() {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) {
            this.updateStatus('Screen sharing not supported in this browser');
            return;
        }
